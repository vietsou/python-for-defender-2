...
class SysmonEvent:
    ...
    def __init__(self, soup):
        ...
        try:
            self.pid = int(SysmonEvent.extract_event_data(soup, "data[name='ProcessId']"))
        except ValueError:
            self.pid = None
        try:
            self.parent_pid = int(SysmonEvent.extract_event_data(soup, "data[name='ParentProcessId']"))
        except ValueError:
...
class NetworkConnect(SysmonEvent):
    ...
    def __init__(self, soup):
        ...
        self.src_port = int(SysmonEvent.extract_event_data(soup, "data[name='SourcePort']"))
        self.dest_port = int(SysmonEvent.extract_event_data(soup, "data[name='DestinationPort']"))
        self.integrity_levels = SysmonEvent.extract_event_data(soup, "data[name='IntegrityLevel']")
...
class ProcessAccess(SysmonEvent):
    """
    Event ID 8
    """
    def __init__(self, soup):
        ...
        self.source_image = SysmonEvent.extract_event_data(soup, "data[name='SourceImage']")
        self.target_image = SysmonEvent.extract_event_data(soup, "data[name='TargetImage']")
        self.granted_access = SysmonEvent.extract_event_data(soup, "data[name='GrantedAccess']")
...
def build_event_soup(soup_evt):
    id = soup_evt.eventid.text
    if id in EVENT_TYPES:
        constructor = EVENT_TYPES[id]
        return constructor(soup_evt)
    return SysmonEvent(soup_evt)

def load_evtx(file_path):
    """
    Generate and return a list of SysmonEvents from EVTX
    """
    records_xml = load_event_xml(file_path)
    return [build_event_xml(r)  for r in records_xml]

def load_xml(file_path):
    """
    Generate and return a list of SysmonEvents from XML format
    """
    with open(file_path) as f:
        soup = BeautifulSoup(f, "lxml")
        events = [s for s in soup.find_all("event")]
        
    return [build_event_soup(e)  for e in events]